...
-- Upgrade Table (Car Modifications)
...
-- Car_Upgrade (M:N Relationship - A Car Can Have Multiple Upgrades)
...
-- Driver_Reward (M:N Relationship - Drivers Can Win Multiple Rewards)
...
-- q1
select D.driver_id,D.first_name,r.race_name from Driver as D
left join Race_Participant as rp
on D.driver_id=rp.driver_id
left join Race as r 
on r.race_id=rp.race_id

--q2
select r.race_name,r.location,rp.driver_id
from Race as r
left join Race_Participant as rp
on r.race_id = rp.race_id;

--q3
select d.driver_id, d.first_name,c.brand,c.model
from Driver as d
left join Driver_Car as dc
on dc.driver_id=d.driver_id
left join car as c 
on c.car_id=dc.car_id

--q4
select c.car_id,c.brand,u.upgrade_id,u.upgrade_name
from Car c
left join Car_Upgrade cu ON c.car_id = cu.car_id
left join Upgrade u ON cu.upgrade_id = u.upgrade_id
...
--q6
select u.upgrade_id,u.upgrade_name,u.upgrade_type,c.car_id,c.model,c.brand
from Upgrade u
left join Car_Upgrade cu ON u.upgrade_id = cu.upgrade_id
left join Car c ON cu.car_id = c.car_id;

--q7
select d.first_name,d1.first_name,dc.car_id from Driver as d join Driver_Car as dc on
d.driver_id=dc.driver_id
join (Driver as d1 
	join Driver_Car as dc2 
	on d1.driver_id=dc2.driver_id )
on dc.car_id=dc2.car_id and d.driver_id<d1.driver_id;

--q8
select d.driver_id, d.first_name
from Driver as d
join Driver_Car as dc on d.driver_id=dc.driver_id
left join Race_Participant rp on d.driver_id=rp.driver_id
where rp.driver_id is null
...
select driver_id ,first_name ,manager
from driver
where driver_id=2

--q10
select d.first_name,d1.first_name,dc.car_id from Driver as d 
join Driver_Car as dc 
on d.driver_id=dc.driver_id
join (Driver as d1 
	join Driver_Car as dc2 
		on d1.driver_id=dc2.driver_id)
on dc.car_id=dc2.car_id 
and d.driver_id<d1.driver_id;

...
--4
select Avg(RunsScored) as average_runs,
Sum(WicketsTaken) as sum_wickets from Cricket

--5
select MissionName 
from SpaceMissions
where DurationDays>=1000 or LaunchYear<1980


--6
select Avg(CaloriesBurned) as avg_cal from Workouts
...
select * from Recipes
where CookingTime<30
and Ingredients like '%Eggs%'
order by CookingTime

--10
select * from Workouts
where (Category='Cardio' or Category='Strength')
and CaloriesBurned>250 and (Duration<( select Avg(Duration) as dur from Workouts))


--11
select Poet,Title ,Theme from Poetry
where (Theme='Love' or Theme='Imagination')
and (Era='Romantic' or Era='Mystical')

--12
SELECT 
    (SUM(DurationDays) / 365) AS Years,
    (SUM(DurationDays) % 365) AS Days,
    CONCAT((SUM(DurationDays) / 365), ' years and ', (SUM(DurationDays) % 365), ' days') AS TotalDuration
FROM SpaceMissions;

--13
select *
FROM Workouts
WHERE CaloriesBurned >= ((SELECT AVG(CaloriesBurned) *0.3 from Workouts)
);
...
SELECT * FROM Cricket;
SELECT * FROM SpaceMissions;
SELECT * FROM PhysicsExperiments;
